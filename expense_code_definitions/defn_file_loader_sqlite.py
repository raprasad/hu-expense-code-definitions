...
#from django.db import connection as db_cursor
...
class SqlLiteDefinitionFileLoader(object):
    """
    ...
    * database tables defined by django models in app expense_code_definitions
    ...
    def load_definitions(self, fullname, EC_CLASS):
        ...
        db_conn = sqlite3.connect(settings.DATABASES['default']['NAME'])
        ...
        with open(fullname, 'rb') as file_input: 
            ...
            if EC_CLASS.__name__ == 'SubActivityValue':
                ...
                insert_stmt = self.get_table_insert_stmt(DEFN_DB_TABLE_NAME, is_subactivity_value=True)
            else:
                db_insert_values = [ self.format_basic_row_info(i) for i in dr]
                insert_stmt = self.get_table_insert_stmt(DEFN_DB_TABLE_NAME)
                
            print '(b) attempt to load --%s-- rows' % len(db_insert_values)

            db_cursor.executemany(insert_stmt, db_insert_values)
            db_conn.commit()        
            db_cursor.close()
    ...
    def format_sub_activity_row_info(self, csv_dict):
        ...
        return (csv_dict['value']\
                , csv_dict['desc'].decode('utf-8', 'ignore')\
                , csv_dict['prefix']\
    ...
    def format_basic_row_info(self, csv_dict):
        return (csv_dict['value']\
                , csv_dict['desc'].decode('utf-8', 'ignore')\
                , csv_dict['prefix']\
                , csv_dict['owning_tub_object']\
    ...
    def process_expense_code_files(self):
        ...
        for file_prefix in self.ec_file_prefixes_ordered:#[:1]:#[5:]:    
            file_cnt +=1
...
if __name__=='__main__':
    #EC_DIR = '/Users/some-username/mcb-git/hu-expense-code-definitions/test_data'
    EC_DIR = '/Users/some-username/HU_GOOGLE_DRIVE/Google Drive/ec_validate/segmentfiles/coa_2011-0713'

    coa_loader = SqlLiteDefinitionFileLoader(EC_DIR)
